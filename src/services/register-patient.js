...
/**
 * Busca todos os medicamentos da promoção no SOAP.
 *
...
async function getMedications() {
  ...
  // TODO: Separar uma função para chamar o SOAP padronizado.
...
/**
 * Traz os medicamentos selecionados pelo paciente no site.
 *
 * @param {*} patient
 * @returns {string[]}
 */
...
/**
 * TODO: Trazer todos os medicamentos que contenham o nome.
 * Filtra os medicamentos selecionados pelo cliente com base no nome.
 *
...
async function generatePatientRequestXML(patient, filteredMedications) {
  const medicationXml = filteredMedications
    .map(
...
/**
 * Converte o JSON do payload para XML.
 *
 * @param {*} payload
 * @returns
 */
async function getPatientRequestXML(payload) {
  const medications = await getMedications();
  const patientMedications = getPatientMedications(payload);
  const filteredMedications = filterMedicationsByName(
    medications,
    patientMedications
  );

  return generatePatientRequestXML(payload, filteredMedications);
}

/**
 * Envia o XML para o SOAP e retorna a resposta.
 *
 * @param {*} xml
 * @returns
 */
async function registerPatient(xml) {
  try {
...
/**
 * Formata a resposta do SOAP em caso de sucesso.
 *
 * @param {*} xml
 */
function parseRegisterPatientResponse(xml) {
  const parser = new XMLParser({
    ignoreAttributes: false,
    attributeNamePrefix: "@_",
  ...
  try {
    ...
    return {
      code: resultado["b:Codigo"],
      message: resultado["b:Mensagem"],
